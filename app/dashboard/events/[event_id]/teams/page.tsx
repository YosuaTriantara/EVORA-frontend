...
export default function TeamsPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [teamsData, categoriesData] = await Promise.all([
        getEventTeamsOrganizer(eventId, { limit: 100 }),
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      setUpdating(true);
      await updateTeamStatus(editingTeam.id, { status: newStatus });
  ...
  const handleUpdateLot = async () => {
    ...
    try {
      setUpdating(true);
      await updateTeamLot(editingTeam.id, { lot_number: parseInt(newLotNumber) });
